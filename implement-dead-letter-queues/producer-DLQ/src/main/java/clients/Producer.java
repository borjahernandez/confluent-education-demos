...
public class Producer {
  ...
  static final String DLQ_TOPIC = "user-topic-dlq";
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    // Creating the Kafka producer for invalid messages
    final Properties settings_DLQ = loadConfig(args[0]);
    settings_DLQ.put(ProducerConfig.CLIENT_ID_CONFIG, "demo-DLQ-producer");
    settings_DLQ.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, ByteArraySerializer.class);
    settings_DLQ.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, StringSerializer.class);

    final KafkaProducer<byte[], String> producer_DLQ = new KafkaProducer<>(settings_DLQ);
    ...
    for (int i = 0; i < NUM_RECORDS; i++) {
      ...
      try {
        final User message_value = User.newBuilder()
        ...
        final ProducerRecord<String, User> record = new ProducerRecord<>(KAFKA_TOPIC, key, message_value);
        ...
        final ProducerRecord<byte[], String> invalidRecord = new ProducerRecord<>(DLQ_TOPIC, invalidMessage);
        producer_DLQ.send(invalidRecord);
      }
